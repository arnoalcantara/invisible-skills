...
- [filosofia.md](filosofia.md) — leis 3 (carga mínima), 4 (hierarquia/ponto focal), 7 (rótulo junto) e 10 (espaço em branco) governam o que entra como imagem.
- [tipologia.md](tipologia.md) — cada ficha traz o campo "decisão visual (roteamento)": o modo default daquele tipo e quando manda (ou não) para gerador.
- [slides-plan-spec.md](slides-plan-spec.md) — você usa este documento para preencher o campo `Visual:` (Modo 0-3 + intenção) de cada slide no plano.

> **Papel deste documento aqui (lado didático).** No plugin `invisible-class-slides-plan` ele serve ao **roteamento/declaração**: decidir qual Modo cada slide declara no campo `Visual:` e a regra de ouro (dado real nunca vai a gerador de imagem). A **execução** (gerar a imagem, style bible aplicado, detecção de gerador em runtime) acontece do lado do renderizador (`invisible-slides-plan-visual`), que tem uma cópia deste mesmo documento. Mudou um, sincronize o outro.
