#!/usr/bin/env python3
"""bootstrap.py — prepara o ambiente da skill invisible-legendas-aplicador.

A skill queima legenda animada com Remotion. Precisa de:
...
Para não instalar node_modules dentro do repo de skills (pesado e versionável),
o projeto Remotion vive num diretório CENTRAL único — ~/.invisible-video/
legendas-remotion — reusado por todos os projetos. Este bootstrap:

  1. detecta node/npm/ffmpeg;
  2. sincroniza os fontes do template (package.json, tsconfig, remotion.config,
     src/) para o diretório central — barato, sempre;
  3. roda `npm install` no central UMA vez (ou de novo se o package.json mudou).

Idempotente: rodar de novo não reinstala à toa.

Uso:
    python3 bootstrap.py [--projeto-dir <dir>] [--check-only]

`--check-only` só reporta o estado, não instala nada.
...
def versao(bin_, flag="--version"):
    try:
        out = subprocess.run(
            [bin_, flag], capture_output=True, text=True, timeout=20
        )
...
def sincronizar_template(template_dir, projeto_dir):
    ...
    # public/ existe pra receber video.mp4 + captions.json em runtime
...
def main():
    ...
    node_ok = estado["node"]["ok"]
    npm_ok = estado["npm"]["ok"]
    ffmpeg_ok = estado["ffmpeg"]["ok"]

    if not node_ok or not npm_ok:
        estado["instrucoes"].append(
            "Node.js + npm ausentes. Instale com `brew install node` (ou nvm)."
        )
    if not ffmpeg_ok:
    ...
    if not args.check_only and node_ok and npm_ok and sources_ok:
        ...
        if not os.path.exists(node_modules) or pkg_mudou:
            ...
            if r.returncode != 0:
                estado["instrucoes"].append(
                    f"npm install falhou em {projeto_dir} — rode manualmente."
                )
    elif not args.check_only:
        # não dá pra instalar, mas ainda sincroniza fontes se possível
        if sources_ok and node_ok:
    ...
    estado["pronto"] = bool(
        node_ok and npm_ok and ffmpeg_ok and estado["projeto_pronto"]
    )
