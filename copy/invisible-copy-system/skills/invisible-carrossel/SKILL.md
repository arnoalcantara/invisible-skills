...
**Quarto caminho — formato `tweet-editorial` (cruzamento).** Quando o material é uma **notícia/fato de mercado com número** (relatório, recorde, pesquisa, movimento de plataforma) e a intenção é virar a cabeça do leitor sobre o que aquilo significa, use o formato híbrido **tweet-editorial**: estrutura de percepção (dado real, recuo histórico, objeção, nomear o conceito) vestida com cadência editorial e disparada por um **fato âncora** (🚨). Método: `base/formatos/carrossel-tweet-editorial.md` (que **puxa** percepção + editorial, não duplica). Saída emite **papel + blocos + ênfases** por card (alvo visual: estilo de render `tweet_editorial`). Recomende-o quando há fato/notícia presente como gatilho; se a virada é atemporal, é percepção pura.
...
4. **Defina o modo** (`autoridade` / `percepcao` / `editorial`) ou o **formato `tweet-editorial`** (quando o material é notícia/fato de mercado com dado), com recomendação. Leia o `base/formatos/carrossel-[modo].md` correspondente + `base/formatos/carrossel.md`. Para tweet-editorial, leia `base/formatos/carrossel-tweet-editorial.md` (que por sua vez puxa `carrossel-percepcao.md` e `carrossel-editorial.md`).
...
- **Só no formato `tweet-editorial`:** vale a regra de dado real do `percepcao` (pesquise e cite, com **dado escalando** quando houver mais de um número); além disso, emita **por card** `papel` (capa/interno/fecho) + os **blocos** presentes (breaking? parágrafos? imagem de prova? cta?) + as **ênfases** marcadas no texto, e uma **descrição de uma linha** onde cabe imagem. Isso é insumo de conteúdo para o render `tweet_editorial`; não é indicação visual (nada de cor, layout, ícone). Os três recursos liberados (🚨 no card 1, swipe no card 1, salvamento no fecho) entram só nos pontos nomeados; o resto segue `portugues-natural.md` (sem travessão, sem emoji solto).
